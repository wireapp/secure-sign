...
public class SwisscomClient {
    ...
    public SignResponse sign(Signer signer, Document document, String hash) throws IOException {
        ...
        inputDocuments.documentHash.documentId = document.name;

        String[] split = signer.name.split(" ");
        String firstName = split[0];
        String lastName = split.length > 1 ? split[1] : split[0];
        certificateRequest.distinguishedName = getDistinguishableName(signer.name, firstName, lastName, signer.pseudonym, "CH");
    ...
    private String getDistinguishableName(String name, String first, String last, String pseudonym, String country) {
        return String.format("CN=TEST %s, givenname=%s, surname=%s, C=%s, emailAddress=%s@wire.com",
                name, first, last, country, pseudonym);
    }
